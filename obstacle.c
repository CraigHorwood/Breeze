...
{
	int wait = o->type == TYPE_GIRDERS_HORIZONTAL ? ((304 - o->w) & 511) : 0;
	if (wait < 0) wait += 512;
	wait += 720 + o->w;
...
char tickObstacle(Obstacle* o, float scale)
{
	...
	if (o->hasGate > 0)
	{
		if ((o->time / o->speed) % o->hasGate == 0)
		{
			if (isOnScreen(o->y, o->y + 96))
			{
				if (o->gate & 128) playSound(SOUND_GATE_OPEN);
				else playSound(SOUND_GATE_CLOSE);
			}
			o->gate ^= 128;
		}
	}
...
void renderObstacle(Obstacle* o)
{
	...
	case TYPE_GIRDERS_VERTICAL:
	{
		render(o->x, 2048 + o->y + o->w, 0.0f, 0.0f, 96.0f, o->w);
		break;
	}
